...
// command + option + enter assitantEditor
    ...
    func initializeView() {
        if let nibView = NSBundle.mainBundle().loadNibNamed("MySlider", owner: self, options: nil).first as? UIView {
    ...
    override func layoutSubviews() {
        ...
        carButton.addTarget(self, action: #selector(MySlider.carButton(_:draggedWithEvent:)), forControlEvents: .TouchDragInside)
        carButton.addTarget(self, action: #selector(MySlider.carButton(_:touchesUpWithEvent:)), forControlEvents: .TouchUpInside)
